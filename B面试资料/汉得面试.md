...
代理模式：动态代理，说白了，其实就是把创建代理对象的过程，用反射来实现，==代理类此时就不需要去实现被代理对象的接口了，因此也就和被代理对象解耦了。==动态代理，不需要把代理类写死了，这样代理的功能就不是固定的了。注意。这里反射出来的代理对象，被代理对象仍然是创建出来然后当作参数传入的。InvocationHandler 通过"**Proxy**"类提供的一个**newProxyInstance**方法用来创建一个对象的代理对象
