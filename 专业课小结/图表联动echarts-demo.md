...
题目链接：http://www.cnsoftbei.com/plus/view.php?aid=715（可下载原始CSV数据集）
...
这个方法实现了**悬停动态效果**，使用 *myChart.on('mouseOver', function (params)*和*myChart.on('mouseOut', function (params)* 鼠标状态监听方法代替了 *myEcharts.on('click',function (params)* 的鼠标点击事件方法。
